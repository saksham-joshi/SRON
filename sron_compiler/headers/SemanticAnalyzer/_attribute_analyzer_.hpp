...
inline static void SemanticAnalyzer::FOUND_ARGS_ATTRIBUTE()
{
    ...
    auto __expected_type1 = SronTokenType::TYPE_OPENING_BRACKETS;
    ...
    while (iterator < tokenVectorEnd)
    {
        ...
        if (iterator->_type == __expected_type1 || iterator->_type == __expected_type2)
        {
            ...
            switch (iterator->_type)
            {
                ...
                case SronTokenType::TYPE_OPENING_BRACKETS:
                    __expected_type1 = SronTokenType::TYPE_STRICT_TYPE;
                    __expected_type2 = SronTokenType::TYPE_DATATYPE;
                    break;

                case SronTokenType::TYPE_DATATYPE:
                case SronTokenType::TYPE_STRICT_TYPE :
                    __expected_type1 = SronTokenType::TYPE_IDENTIFIER;
                    __expected_type2 = SRON_NO_TYPE;
                    break;

                case SronTokenType::TYPE_IDENTIFIER: ++count_of_parameter;
                    iterator->_cache_value = SemanticAnalyzer::variableSet.insert(SronHasher(iterator->_token.c_str()));
                    __expected_type1 = SronTokenType::TYPE_COMMA;
                    __expected_type2 = SronTokenType::TYPE_CLOSING_BRACKETS;
                    break;

                case SronTokenType::TYPE_COMMA:
                    __expected_type1 = SronTokenType::TYPE_DATATYPE;
                    __expected_type2 = SronTokenType::TYPE_STRICT_TYPE;
                    break;
        ...
        {
            throw ThrowException("analysing the code. 'args' attribute has invalid syntax. Expected a value of type '" + std::string(Support::TYPE_TO_STRING(__expected_type1)) + "' but found '" + iterator->_token + "'", SronError::InvalidArgsSyntaxError, iterator->_file_index);
        }
...
inline static void SemanticAnalyzer::FOUND_MAIN_ARGS_ATTRIBUTE()
{
    ...
    if(iterator < tokenVectorEnd-4 && (iterator->_type == SronTokenType::TYPE_OPENING_BRACKETS 
                                   && ((iterator+1)->_type == SronTokenType::TYPE_DATATYPE && (iterator+1)->_token == "List") 
                                   && (iterator+2)->_type == SronTokenType::TYPE_IDENTIFIER 
                                   && (iterator+3)->_type == SronTokenType::TYPE_CLOSING_BRACKETS))
    {
...
inline static void SemanticAnalyzer::FOUND_NAME_ATTRIBUTE()
{
    ...
    const auto cur_pos = iterator - SemanticAnalyzer::tokenVectorBegin;

    if(cur_pos != 1 && cur_pos != 2) throw ThrowException("analysing the code. The first expected attribute in a function is 'name'");
    
    iterator += 2;

    if (iterator->_type != SronTokenType::TYPE_IDENTIFIER)
    {
        throw ThrowException("analyzing the code. Expected a valid function name but found '" + iterator->_token + "'");
    }
    ...
    {
        throw ThrowException("analysing the code. The function name '" + iterator->_token + "' is a reserved file name in windows system", SronError::InvalidFunctionNameError, iterator->_file_index);
    }
...
inline static void SemanticAnalyzer::FOUND_ROTATE_ATTRIBUTE()
{
    ...
    while (iterator < tokenVectorEnd && iterator->_type != SronTokenType::TYPE_CLOSING_BRACKETS)
    {
...
inline static void SemanticAnalyzer::FOUND_CONDITION_ATTRIBUTE()
{
    ...
    default:
        throw ThrowException("analyzing the code. Expected a value inside the 'condition' attribute which can return either 'true' or 'false' but found '" + iterator->_token + "'", iterator->_file_index);
    }
}
inline static void SemanticAnalyzer::FOUND_RANGE_ATTRIBUTE()
{
    ...
    try{
        ...
        if (iterator->_type != SronTokenType::TYPE_OPENING_BRACKETS)
        {
            ...
            throw "analysing the 'range' attribute. Found '" + iterator->_token + "' but expected an opening round bracket";
        }
        ...
        if (iterator->_type == SronTokenType::TYPE_DATATYPE)
        {
            SemanticAnalyzer::FOUND_TYPE_DATATYPE();
        }
        else if (iterator->_type == SronTokenType::TYPE_IDENTIFIER)
        {
            SemanticAnalyzer::FOUND_TYPE_IDENTIFIER();
        }
        else
        {
            throw "analyzing the code, Expected an identifier or a variable declaration but found '" + iterator->_token + "'";
        ...
        if (iterator->_type == SronTokenType::TYPE_COMMA)
        {
            // nothing to do
        }
        else if (iterator->_type == SronTokenType::TYPE_ASSIGN)
        {
            ...
            if (iterator->_type != SronTokenType::TYPE_COMMA)
            {
                throw "analyzing the code. Expected a comma(,) but found '"+ iterator->_token +"'";
                
            }
        }
        else
        {
            throw "analyzing the code, Expected an comma(,) or a assign(=) but found '" + iterator->_token + "'";
        }
        ...
        if (iterator->_type == SronTokenType::TYPE_CLOSING_BRACKETS)
        {
            return;
        }
        else if (iterator->_type == SronTokenType::TYPE_COMMA)
        {
            ++iterator;
        ...
        if (iterator->_type != SronTokenType::TYPE_CLOSING_BRACKETS)
        {
            throw "analyzing the code. Expected an closing bracket ')' but found '" + iterator->_token + "'";
        }
...
inline static void SemanticAnalyzer::FOUND_CONSOLE_ATTRIBUTE()
{
    ...
    if (iterator->_type != SronTokenType::TYPE_OPENING_BRACKETS)
    {
        throw ThrowException("analysing the code. Expected a '(' but found '" + iterator->_token + "'", iterator->_file_index);
    }
    ...
    {
        if ( iterator->_flag == BytecodeFlags::Flag_OBuffer ) { }
        ...
        else if (iterator->_type == SronTokenType::TYPE_CLOSING_BRACKETS)
            break;

        throw ThrowException("analysing the code. Expected a ',' or ')' but found '" + iterator->_token + "'. A better syntax can help you to debug any unexpected errors during compilation or runtime", iterator->_file_index);
    }
}
...
inline static void SemanticAnalyzer::FOUND_RETURN_ATTRIBUTE()
{
    ...
    if(iterator >= SemanticAnalyzer::tokenVectorEnd) throw ThrowException("analyzing the code. Expected  a value type after 'return' attribute but function ended", iterator->_file_index);

    SemanticAnalyzer::ImplicitConversion::THROW_EXCEPTION_IF_RETURNED_VALUE_DOESNOT_MATCH_WITH_RETURN_TYPE();
...
inline static void SemanticAnalyzer::FOUND_LOOP_ATTRIBUTE()
{
    ...
    const auto& loop_attr = *iterator;

    iterator += 2;

    SemanticAnalyzer::THROW_ERROR_IF_ITERATOR_NOT_WITHIN_BOUND();

    if (iterator->_type != SronTokenType::TYPE_FUNCTION_SCOPE_START)
    {
        throw ThrowException("analysing the code. Expected a scope opening '{' after '" + loop_attr._token + "' attribute but found '" + iterator->_token + "'", iterator->_file_index);
    }
    ...
    if (iterator->_type == SronTokenType::TYPE_NEWLINE)
    {
        iterator += 1;
        SemanticAnalyzer::THROW_ERROR_IF_ITERATOR_NOT_WITHIN_BOUND();
    }

    if (loop_attr._flag == BytecodeFlags::Flag_ForScopeStart)
    {
        if (iterator->_flag == BytecodeFlags::Flag_RangeScopeStart) SemanticAnalyzer::FOUND_RANGE_ATTRIBUTE();
           
        else throw ThrowException("analysing the code. Expected 'range' attribute after 'for' but found '" + iterator->_token + "'", iterator->_file_index);
    }
    else if (loop_attr._flag == BytecodeFlags::Flag_WhileScopeStart)
    {
        if (iterator->_flag == BytecodeFlags::Flag_Condition) SemanticAnalyzer::FOUND_CONDITION_ATTRIBUTE();
        
        else throw ThrowException("analysing the code. Expected 'condition' attribute after 'while' but found '" + iterator->_token + "'", iterator->_file_index);
    }
    else if(loop_attr._flag == BytecodeFlags::Flag_ForeachScopeStart)
    {
        if (iterator->_flag == BytecodeFlags::Flag_ForeachOn) SemanticAnalyzer::FOUND_FOREACH_ON_ATTRIBUTE();
          
        else throw ThrowException("analysing the code. Expected 'on' attribute after 'foreach' but found '" + iterator->_token + "'", iterator->_file_index);
    }
}
...
inline static void SemanticAnalyzer::FOUND_IF_ELIF_ELSE_ATTRIBUTE()
{
    ...
    if (iterator->_type != SronTokenType::TYPE_FUNCTION_SCOPE_START)
    {
        throw ThrowException("analysing the code. Expected a scope opening '{' after '" + std::string((iterator - 2)->_token.c_str()) + "' attribute but found '" + iterator->_token + "'", iterator->_file_index);
    }
    ...
    else if (__attribute_name == AttributeElif || __attribute_name == AttributeIf)
    {
        ...
        if (iterator->_type == SronTokenType::TYPE_ATTRIBUTE && iterator->_token == AttributeCondition)
        {
        ...
        throw ThrowException("analysing the code. Expected 'condition' attribute but found '" + iterator->_token + "'", iterator->_file_index);
    }
}
...
inline static void SemanticAnalyzer::FOUND_ATTRIBUTE_TYPE(){
    ...
    if ( SemanticAnalyzer::fncReturnType != BytecodeFlags::Flag_bydefault )
    {
    ...
    if(iterator->_type != SronTokenType::TYPE_DATATYPE)
    {
        throw ThrowException("analyzing the code. Expected a datatype after 'type' attribute but found " + iterator->_token +" . Correct syntax- type : Int",iterator->_file_index);
    }
...
inline static void SemanticAnalyzer::FOUND_FOREACH_ON_ATTRIBUTE()
{
    ...
    if(iterator < SemanticAnalyzer::tokenVectorEnd && iterator->_type != SronTokenType::TYPE_OPENING_BRACKETS){
        throw ThrowException("analyzing the code. Expected a '(' after 'on : ' but found '"+ iterator->_token + "'", iterator->_file_index);
    }
    ...
    switch(iterator->_type){
        ...
        case SronTokenType::TYPE_STRING : break;
        ...
        default : throw ThrowException("analyzing the code. Expected a collective type value (eg. String or List) but found '" + iterator->_token +"'", iterator->_file_index);
    }
    ...
    if(iterator->_type != SronTokenType::TYPE_CLOSING_BRACKETS){
        throw ThrowException("analyzing the code. Expected a ')' at the end of 'on : ' but found '"+ iterator->_token + "'", iterator->_file_index);
    }
    
}

inline static void SemanticAnalyzer::FOUND_OS_ATTRIBUTE()
{
    using SemanticAnalyzer::iterator;

    iterator += 2;

    // Now the position of the iterator is on the token after colon(:)
    if ( iterator->_type != SronTokenType::TYPE_FUNCTION_SCOPE_START)
    {
        throw ThrowException("analyzing the code. Expected a '{' after attribute '"+std::string((iterator-2)->_token)+"''s colon(:) but found '"+ std::string(iterator->_token) +"'", SronError::IncorrectOsAttributeError, iterator->_file_index);
    }

    SemanticAnalyzer::scope_stack.push(SemanticAnalyzer::SRON_OS_SCOPE);
