...
inline static bool SemanticAnalyzer::CHECK_IF_USER_DEFINED_FUNCTION_EXISTS(const SronTokenString &fnc_name) noexcept
{

    std::string fnc_name_without_dots = fnc_name.GET();

    std::replace(fnc_name_without_dots.begin(), fnc_name_without_dots.end(), '.', '\\');

    // checking if the function is=available in the set of user defined functions....
    using SemanticAnalyzer::userDefinedFncSet;

    if (userDefinedFncSet.find(SronHasher::HashFunction()(fnc_name_without_dots.c_str())) != userDefinedFncSet.end())
    {
        return true;
    }

    // if it is not a user-defined function, then we should search on file directories....
    std::ifstream file(Logs::directory_path + fnc_name_without_dots + ".srb");

    if (!file.fail())
    {
        userDefinedFncSet.insert(SronHasher::HashFunction()(fnc_name_without_dots.c_str()));
        return true;
    }
