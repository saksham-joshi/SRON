...
inline List::List(List::SronList_t&& __val) : value(std::move(__val))
{
...
inline AnyPtr List::copy() const
{
    ...
    for(auto i : this->value) lst->push(i->ptr());
...
inline void List::fillObuffer(SronOBuffer& __buffer) const
{
    ...
    auto prepare_buffer = [ &__buffer , &it ] ( ) {
        ...
        switch ( (*it)->type() )
        {
            case SronTypeNum::TYPE_CHAR :   __buffer << '\'' << (*it)->getBaseChar()   << '\'' ; break;

            case SronTypeNum::TYPE_STRING : __buffer << '\"' << (*it)->getBaseStr() << '\"' ; break;

            case SronTypeNum::TYPE_LITERAL_STR : __buffer << '\"' << (*it)->getBaseLstr() <<"\""; break;

            default : (*it)->ptr()->fillObuffer( __buffer );
        }
    };

    for (; it != end && SronLogs::OKAY__; ++it)
    {
        prepare_buffer();
        __buffer << ", ";
    }
...
inline List::SronList_t& List::getBaseList() noexcept
{
    return this->value;
}
...
inline AnyBindPtr List::at(Int &index)
{
    ...
    throwExceptionIfIndexOutOfBound(index.getBaseInt(), this->value.size(), "extraction", "List");

    return this->value.at(index.getBaseInt());
}

inline long long int List::count(AnyBind &val)
{

    long long int _count = 0;

    SronTypeNum type_of_val = val.type();
...
{
    throwExceptionIfIndexOutOfBound(index.getBaseInt(), this->value.size(), "deletion", "List");

    auto pos = this->value.begin() + (index.getBaseInt());
...
inline long long int List::index(AnyBind &val) const
{
...
inline void List::insert(Int &index, AnyBind &val)
{
    throwExceptionIfIndexOutOfBound(index.getBaseInt(), this->value.size(), "insertion", "List");

    this->value.insert((this->value.begin() + (index.getBaseInt())), new AnyBind(val->copy() , val->typeNum(), this->layer));
}
...
inline void List::replace(AnyBind &replacer, AnyBind &replacement)
{
    ListPtr list = new List();

    SronTypeNum type_of_replacement = replacement.type();

    SronTypeNum type_of_replacer = replacer.type();
...
inline long long int List::rIndex(AnyBind &val) const
{
    SronTypeNum type_of_val = val.type();
...
{
    throwExceptionIfIndexOutOfBound(start_index.getBaseInt(), this->value.size(), "extraction", "List");
    throwExceptionIfIndexOutOfBound(end_index.getBaseInt(), this->value.size(), "extraction", "List");

    auto i_1 = this->value.begin() + start_index.getBaseInt();
    auto i_2 = this->value.begin() + end_index.getBaseInt();
...
inline void List::update(Int &index, AnyBind &val)
{
    throwExceptionIfIndexOutOfBound(index.getBaseInt(), this->value.size(), "updation", "List");

    auto value = this->value[index.getBaseInt()];
...
inline AnyPtr List::__add__(Any &__val , SronTypeNum __type) const
{
    switch (__type)
    {
        ...
        case SronTypeNum::TYPE_LIST: {
            ListPtr lst = listCast(this->copy());
