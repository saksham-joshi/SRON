...
constexpr inline const unsigned short int PostFixGenerator::PRECEDENCE(const flag_t __flag)
{
    switch (__flag)
    {
        case Flag_Not:
...
inline TokenVector::iterator PostFixGenerator::POSTFIX()
{
    ...
    while (this->vecit < tokenVectorEnd && vecit->_flag != Flag_EvalEnd)
    {
            ...
            std::cout << "\n----------->>>>>>>> '" << this->vecit->_token << "'    '" << Support::FLAG_TO_STRING(this->vecit->_flag) << "'" ;
        #endif
        ...
        {
            throw ThrowException("parsing and optimizing a mathematical expression. Found this token '" + std::string(vecit->_token.GET()) + "' of type '" + std::string(Support::TYPE_TO_STRING(vecit->_type)) + "'", MathematicalBlockSyntaxException);
        }
