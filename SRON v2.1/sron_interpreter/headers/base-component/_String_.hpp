...
inline const unsigned short int String::TYPE_NUMBER() const
{
    return TYPE_STRING;
}
...
inline ListPtr String::GET_LIST() const
{
    ...
    {
        lst->PUSH_WITHOUT_COPY(new Char(i));
    }

    GarbageCollector::REGISTER_MEMORY(lst);
...
{
    return GarbageCollector::REGISTER_MEMORY(new Char(this->value[__index]));
}

// clears the string value, the length of string will be zero after calling this function
...
inline void String::PUSH_WITHOUT_COPY(AnyPtr) {}
...
inline AnyBindPtr String::AT(Int &index)
{
    ...
    return GarbageCollector::REGISTER_MEMORY(new Char(this->value.at(*index)));
}
...
inline long long int String::INDEX(Any &val) const
{
    ...
    case TYPE_CHAR:
    {
        auto x = this->value.find(**val.GET_CHAR());
        return (x == std::string::npos) ? -1 : x;
    }

    case TYPE_STRING:
    {
        auto x = this->value.find(**val.GET_STRING());
        return (x == std::string::npos) ? -1 : x;
    }
    }

    throw ThrowException("searching a value of type '" + std::string(val.TYPE()) + "' in 'String'. Expected 'String' or 'Char'", ArgumentException);
}
...
inline AnyBindPtr String::POP()
{
    ...
    return GarbageCollector::REGISTER_MEMORY(new Char(ch));
}
...
inline void String::REPLACE(Any &replacer, Any &replacement)
{
    ...
    case TYPE_CHAR:
        this->REPLACE(replacer.TO_STRING(), replacement.TO_STRING());
        break;

    default:
        throw ThrowException("performing String replacment on a value of type '" + std::string(replacer.TYPE()) + "'. Expected 'Char' or 'String' type values", ArgumentException);
    }
}
...
inline void String::SET(AnyBind &val)
{

    this->value = val->TO_STRING();
}
...
inline ListPtr String::SPLIT(Char &_splitter)
{
    ...
    for (const auto &i : this->value)
    {
        if (i == *_splitter)
        {
            lst->PUSH_WITHOUT_COPY(new String(temp));
            temp = "";
        }
        else
        {
...
inline AnyBindPtr String::SUBPART(Int &start_index, Int &end_index) 
{
    ...
    return GarbageCollector::REGISTER_MEMORY(ptr);
}
...
inline AnyPtr String::__ADD__(Any &val) const
{
    return new String(this->TO_STRING().append(val.TO_STRING()));
}

inline bool String::__IS_EQUAL__(Any &val) const
{
    if (val.TYPE_NUMBER() == TYPE_STRING)
    {
        return this->value == **val.GET_STRING();
    }
    return false;
}

inline bool String::__NOT_EQUAL__(Any &val) const
{
    if (val.TYPE_NUMBER() == TYPE_STRING)
    {
        return this->value != **val.GET_STRING();
...
inline bool String::__LESS_THAN__(Any &val) const
{
    if (val.TYPE_NUMBER() == TYPE_STRING)
    {
        return this->value < **val.GET_STRING();
...
inline bool String::__LESS_THAN_EQUAL__(Any &val) const
{
    if (val.TYPE_NUMBER() == TYPE_STRING)
    {
        return this->value <= **val.GET_STRING();
...
inline bool String::__MORE_THAN__(Any &val) const
{
    if (val.TYPE_NUMBER() == TYPE_STRING)
    {
        return (this->value > (**val.GET_STRING()));
...
inline bool String::__MORE_THAN_EQUAL__(Any &val) const
{
    if (val.TYPE_NUMBER() == TYPE_STRING)
    {
        return (this->value >= (**val.GET_STRING()));
...
inline void String::__ADD_AND_ASSIGN__(Any &val)
{
    this->value.append(val.TO_STRING());
}
