    ...
    using SemanticAnalyzer::iterator;
        ...
        for (; iterator < SemanticAnalyzer::tokenVectorEnd; ++iterator)
        {
            ...
            switch (iterator->_type)
            {
                ...
                case TYPE_OPERATOR : throw ThrowException("analysing the code. Found '" + std::string(iterator->_token.c_str()) + "' operator outside the mathematical block", iterator->_file_index);

                case TYPE_STRICT_TYPE : throw ThrowException("analysing the code. Found '"+std::string(iterator->_token.c_str())+"' which can only be write in 'args' attribute", iterator->_file_index);
        ...
        const char*const fnc_name = ByteCodeWriter::fnc_name->c_str();
            ...
            for(const auto& i : __tokenVector){
                printf_s ("\n--{ Token= '%s'     Flag='%s'    }--", i._token.c_str() , Support::FLAG_TO_STRING(i._flag));
            }
        #endif
...
inline static void SemanticAnalyzer::RESET() noexcept
{
    ...
    SemanticAnalyzer::fncReturnType = Flag_bydefault;
}
...
inline static void SemanticAnalyzer::VALUE_TYPE_CHECK()
{
    ...
    switch (iterator->_type)
    {
        ...
        case TYPE_KEYWORD:
            if ( iterator->_flag == Flag_OBuffer ) throw ThrowException("analyzing the code. Found '" + std::string( iterator->_token.c_str()) + "' which cannot be used as a value. It's a 'Keyword'", SronError::InvalidOBufferSyntaxError, iterator->_file_index);
        default:
...
inline static unsigned short int SemanticAnalyzer::VERIFY_BYTECODE_FILE( const char* __fnc_name ) noexcept
{
    ...
    fopen_s( &file , ( Logs::directory_path + str ).c_str() , "rb");
    ...
    // reading the meta 
    fread_s( &meta , sizeof(meta) , sizeof(meta), 1 , file );
