...
inline static void SemanticAnalyzer::FOUND_TYPE_ATTRIBUTE()
{
    ...
    switch(iterator->_flag){
        ...
        case Flag_FunctionParamScopeStart : SemanticAnalyzer::FOUND_ARGS_ATTRIBUTE(); break;
...
inline static void SemanticAnalyzer::FOUND_TYPE_KEYWORD()
{
    ...
    else if ( iterator->_flag == Flag_OBuffer )
    {
        ...
        if ( iterator->_flag != Flag_AddAssign ) throw ThrowException("analyzing the code. Expected '+=' after 'OBUFFER' but found '" + std::string( iterator->_token.c_str()) + "'", SronError::InvalidOBufferSyntaxError, iterator->_file_index);
    ...
    try
    {
        ...
        auto switch_tree = []()
        {
            ...
            { // operand only
                ...
                case TYPE_IDENTIFIER_WITH_DOT: throw ThrowException("analysing the mathematical block. '" + std::string(iterator->_token.c_str()) + "' is expected to be a function call but used as an identifier" , SemanticAnalyzer::iterator->_file_index);
