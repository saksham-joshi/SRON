...
#define DatatypeConversionError "> Reason : You input invalid character."
...
#define RectifyingException "> Reason : Interpreter misunderstood the value.\n\n\t|> Solution : Contact Saksham Joshi via Linkedin or Twitter(X) to fix this problem.\n"
...
#define VectorEmptyException "> Reason : Vector is empty so no operation is possible in it.\n\n\t|> Solution : Try to put a conditional statement in your code stating that if the Vector\'s Length is 0, then No operation can be done on it."
...
// void DISPLAY_EXCEPTION(const std::string during , const int code){
//     printf("heree");
...
void DISPLAY_EXCEPTION(const char *during, const int code){
    ...
    switch (code){
        ...
        case 23 :
            ...
            break;
        case 24 :
            printf("%s",VectorEmptyException);
            break;
        case 25 :
            printf("%s",RectifyingException);
            break;
        default:
