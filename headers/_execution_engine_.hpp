...
inline namespace ExecutionEngine
{
    ...
    static ExecutionEngineFuncMap _funcmap_{
        ...
        {Flag_ElseScopeStart, ExecutionEngine::EXECUTE_CONDITIONAL_STATEMENT},
    ...
    inline static Any *ASSIGN_VALUE()
    {
        try
        {
            ...
            // if(ExecutionEngine::_FunctionStack_.top()->_Vmanager.TYPE_NUMBER(variable_name) != value->TYPE_NUMBER() ){
    ...
    /* This function will move the iterator to the most recent Loop opening flag.
    ...
    */
    ...
    inline static Any *EXTRACT_STRING()
    {
        ...
        while (*it < end && **it != Flag_StringScopeEnd)
        {
            str->APPENDS(**it + '\n');
            ++(*it);
        }
    ...
    inline static Any *EXECUTE_FOR_STATEMENT()
    {
        ...
        // std::cout << "\n--------->start" << loop_start->TO_STRING()<<"<" ;
        // std::cout << "\n--------->end" << loop_end->TO_STRING()<<"<" ;
    ...
    inline static Any *RETURN_VALUE()
    {
        ...
        ++ExecutionEngine::_FunctionStack_.top()->_iterator;
