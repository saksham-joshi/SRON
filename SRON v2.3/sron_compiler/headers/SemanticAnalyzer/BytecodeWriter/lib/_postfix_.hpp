...
class PostFixGenerator
{
    ...
    constexpr inline static unsigned short int PRECEDENCE(const BytecodeFlags __flag);
...
constexpr inline unsigned short int PostFixGenerator::PRECEDENCE(const BytecodeFlags __flag)
{
    ...
    throw ThrowException("optimizing mathematic block. No precedence for operator for '" + std::string(SronSupport::flagToString(__flag)) + "' found");
}
