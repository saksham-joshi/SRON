...
{
    constexpr static FilePos_t _DEFAULT_FILE_INDEX = UINT32_MAX;

    std::string _message;
    const char* _excode;
    FilePos_t _file_index;

    explicit inline ThrowException(const char *const __message, const char *const excode = SronError::DefaultError,  const FilePos_t __file_index = ThrowException::_DEFAULT_FILE_INDEX)
        : _message(__message), _excode(excode), _file_index(__file_index) {}

    explicit inline ThrowException(const std::string &__message, const char *const excode = SronError::DefaultError, const FilePos_t __file_index = ThrowException::_DEFAULT_FILE_INDEX)
        : _message(__message), _excode(excode), _file_index(__file_index) {}


    explicit inline ThrowException(const char* const __message, FilePos_t __file_index, const char*const __excode = SronError::DefaultError)
        : _message(__message) , _excode(__excode) , _file_index(__file_index) { }
    
    explicit inline ThrowException(const std::string&__message, FilePos_t __file_index, const char*const __excode = SronError::DefaultError)
        : _message(__message) , _excode(__excode) , _file_index(__file_index) { }
