...
namespace LexicalAnalyzer
{
    ...
    inline static void UPDATE_TOKEN_OBJECT(const SronTokenType __type, const char *const __token, BytecodeFlags __flag) noexcept
    {
    ...
    */
    ...
    inline static void FOUND_ALPHABET_OR_UNDERSCORE()
    {
        ...
        else if (LexicalAnalyzer::GET_LAST_TOKEN_ADDRESS()->_type == SronTokenType::TYPE_DATATYPE)
        {
            ...
            LexicalAnalyzer::UPDATE_TOKEN_OBJECT(SronTokenType::TYPE_IDENTIFIER, temp_string.c_str(), BytecodeFlags::Flag_Identifier_Variable);
        }
        else
        {
            LexicalAnalyzer::UPDATE_TOKEN_OBJECT(SronTokenType::TYPE_IDENTIFIER, temp_string.c_str(), BytecodeFlags::Flag_Identifier_Variable);
    ...
    inline static void FOUND_ASSIGN()
    {
        ...
        if ((__last_token._type == SronTokenType::TYPE_OPERATOR || __last_token._type == SronTokenType::TYPE_ASSIGN) && __last_token._token.length() == 1)
        {
            ...
            __last_token._file_index = LexicalAnalyzer::GET_SOURCE_CODE_INDEX();
        }
        else if (__last_token._type == SronTokenType::TYPE_OPERATOR && __last_token._flag == BytecodeFlags::Flag_Compare)
        {

            __last_token._token += "=";
            __last_token._flag = BytecodeFlags::Flag_StrictCompare;
            __last_token._file_index = LexicalAnalyzer::GET_SOURCE_CODE_INDEX();
        }
        else
        {
            LexicalAnalyzer::UPDATE_TOKEN_OBJECT(SronTokenType::TYPE_ASSIGN, "=", BytecodeFlags::Flag_Assign);
    ...
    inline static void FOUND_DOUBLE_QUOTE()
    {
        ...
        if ( temp_string.length() >= BYTECODE_STRING_MAX_LEN ) throw ThrowException("tokenizing the code. The length of the constant string is exceeding the limit", SronError::TooLargeStringError, LexicalAnalyzer::GET_SOURCE_CODE_INDEX());


        LexicalAnalyzer::UPDATE_TOKEN_OBJECT(SronTokenType::TYPE_STRING, temp_string.c_str(), BytecodeFlags::Flag_String_Value);
    }
    ...
    inline static void FOUND_MINUS()
    {
        ...
        case SronTokenType::TYPE_CLOSING_BRACKETS:
            LexicalAnalyzer::UPDATE_TOKEN_OBJECT(SronTokenType::TYPE_OPERATOR, "-", BytecodeFlags::Flag_Minus);
            break;

        default:
            LexicalAnalyzer::UPDATE_TOKEN_OBJECT(SronTokenType::TYPE_OPERATOR, "-", BytecodeFlags::Flag_Negate); // negate operator
        }
    }
    ...
    inline static void FOUND_NEWLINE()
    {
        ...
        LexicalAnalyzer::UPDATE_TOKEN_OBJECT(SronTokenType::TYPE_NEWLINE, "\n", BytecodeFlags::Flag_bydefault);
    }
    ...
    inline static void FOUND_NUMBER()
    {
        ...
        while (iterator < LexicalAnalyzer::fileEnd && valid_numeric_character(*iterator))
        {
            ...
            else if (is_double_type_value && *iterator == '.')    throw ThrowException("tokenizing the code. Found '" + (temp_string + *iterator) + "' which contains more than one decimal points", SronError::DefaultError, LexicalAnalyzer::GET_SOURCE_CODE_INDEX());
            
            // LLONG_MAX has length of 19 and if the value contains '-', then the length becomes 20
            else if ( temp_string.length() >= 21 )    throw ThrowException("tokenizing the code. Found '" + temp_string + "' is longer than enough" , SronError::TooLargeNumberError, LexicalAnalyzer::GET_SOURCE_CODE_INDEX());
        ...
        // LexicalAnalyzer::UPDATE_TOKEN_OBJECT((is_double_type_value ? TYPE_DOUBLE : TYPE_INT), temp_string.c_str(), ((is_double_type_value) ? BytecodeFlags::Flag_Double_Value : BytecodeFlags::Flag_Int_Value));
        ...
        {
            SemanticAnalyzer::THROW_ERROR_IF_DIVISION_BY_ZERO_FOUND(temp_string, LexicalAnalyzer::GET_SOURCE_CODE_INDEX());
        }
    ...
    inline static void FOUND_SCOPE_OPERATOR()
    {
        ...
        {
            throw ThrowException("generating bytecode and handling scopes. Found '" + temp_string + "'", SronError::InvalidScopeError, LexicalAnalyzer::GET_SOURCE_CODE_INDEX());
        }
    }
    ...
    inline static void FOUND_SINGLE_QUOTE()
    {
        ...
        {
            LexicalAnalyzer::UPDATE_TOKEN_OBJECT(SronTokenType::TYPE_CHAR, " ", BytecodeFlags::Flag_Char_Value);
        }
        else
        {
            temp_string += *iterator;
        ...
        LexicalAnalyzer::UPDATE_TOKEN_OBJECT(SronTokenType::TYPE_CHAR, temp_string.c_str(), BytecodeFlags::Flag_Char_Value);
    }
    ...
    inline static void FOUND_SCOPE_CLOSE()
    {
        ...
        if(LexicalAnalyzer::scope_stack.empty()){
            throw ThrowException("tokenizing the code and handling scopes", SronError::InvalidScopeError , LexicalAnalyzer::GET_SOURCE_CODE_INDEX());
        }
        ...
        if( LexicalAnalyzer::scope_stack.top() == '{')
        {
            ...
            throw ThrowException("tokenizing the code and handling scope for '}'", SronError::InvalidScopeError , LexicalAnalyzer::GET_SOURCE_CODE_INDEX());
        }
    ...
    inline static void FUNCTION_TOKENIZATION_DONE()
    {
            ...
            for (const auto &i : LexicalAnalyzer::tokenVector){
                std::printf ("<[ Token=`%s`    Type=`%s`    Flag=`%s` ]>", i._token.c_str(), Support::TYPE_TO_STRING(i._type), SronSupport::flagToString(i._flag) );
            }
        #endif
    ...
    inline static void TOKENIZER()
    {
        ...
        for (; iterator < fileEnd; ++iterator)
        {
            ...
            else if (*iterator == '\n')
            {
                ...
                if (LexicalAnalyzer::GET_LAST_TOKEN_ADDRESS()->_type == SronTokenType::TYPE_NEWLINE)
                {
            ...
            else if (*iterator == ':')
            {
                ...
                LexicalAnalyzer::UPDATE_TOKEN_OBJECT(SronTokenType::TYPE_COLON, ":", BytecodeFlags::Flag_bydefault);
            }
            else if (*iterator == '~')
            {
            ...
            else
            {
                ...
                throw ThrowException("tokenizing the code and found an invalid token '" + LexicalAnalyzer::temp_string + "'", SronError::InvalidTokenError, LexicalAnalyzer::GET_SOURCE_CODE_INDEX());
            }
        ...
        if (!scope_stack.empty())
        {
            throw ThrowException("tokenizing the code", SronError::InvalidScopeError, LexicalAnalyzer::GET_SOURCE_CODE_INDEX());
        }
    }
