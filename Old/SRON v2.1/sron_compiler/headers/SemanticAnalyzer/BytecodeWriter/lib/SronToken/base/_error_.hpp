...
#define NoException "DefaultSyntaxError"
...
inline static void DISPLAY_ERROR(const char*const &during, const char *const &code, unsigned long __line_number = ULONG_MAX)
{
    ...
    std::cerr << "    |> Read Documentations at https://sron.gitbook.io <|";
}
...
// inline static void DISPLAY_WARNING(const std::string &_warning)
// {
//     std::cerr << "\n-----------------------------\nWarning in '" + Logs::filename + "': \nLine number: "  << '\n'
//               << _warning << "\n-----------------------------";
// }
