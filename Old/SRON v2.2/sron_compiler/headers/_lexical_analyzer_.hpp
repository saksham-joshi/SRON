...
namespace LexicalAnalyzer
{
    ...
    */
    ...
    inline static void FOUND_ASSIGN()
    {
        ...
        if ((__last_token._type == TYPE_OPERATOR || __last_token._type == TYPE_ASSIGN) && __last_token._token.length() == 1)
        {
    ...
    inline static void FUNCTION_TOKENIZATION_DONE()
    {
            ...
            for (const auto &i : LexicalAnalyzer::tokenVector){
                printf_s ("<[ Token=`%s`    Type=`%s`    Flag=`%s` ]>", i._token.c_str(), Support::TYPE_TO_STRING(i._type), Support::FLAG_TO_STRING(i._flag) );
            }
        #endif
    ...
    inline static void UPDATE_LAST_TOKEN_IF_IDENTIFIER()
    {
        ...
        if (__last_token._type == TYPE_IDENTIFIER || __last_token._type == TYPE_IDENTIFIER_WITH_DOT)
        {
            ...
            {
                __last_token._flag = Flag_InBuiltFunctionCall;
                __last_token._cache_value = it->_index;
            }
            else 
            {
                __last_token._flag = Flag_UserDefinedFunctionCall;
            }
        }
    }
